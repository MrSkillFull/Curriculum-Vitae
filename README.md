# 👋 Fernando Lima — Desenvolvedor Fullstack & Especialista em Automação

[![Currículo Online](https://img.shields.io/badge/🔗%20Ver%20Currículo%20Online-22d3ee?style=for-the-badge)](https://curriculum-vitae-fernando-lima-dev.vercel.app/)
[![Status](https://img.shields.io/badge/Status-Disponível%20para%20Oportunidades-22d3ee?style=for-the-badge)](https://www.linkedin.com/in/fernandolima-dev)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-fernandolima--dev-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/fernandolima-dev)
[![Email](https://img.shields.io/badge/Email-Contato-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:fernandoassisoliveiralima@gmail.com)

> **🌐 Acesse agora:** [curriculum-vitae-fernando-lima-dev.vercel.app](https://curriculum-vitae-fernando-lima-dev.vercel.app/)

## Sobre Mim

Estudante de **Análise e Desenvolvimento de Sistemas** e técnico em Administração, com experiência prática em multinacionais do setor de Óleo e Gás. Minha atuação combina desenvolvimento de software, automação de processos (RPA) e análise de dados para gerar eficiência real em ambientes corporativos.

Busco oportunidades em **desenvolvimento de software**, **RPA**, **análise de processos** ou **automação de TI**.

## 🛠️ Competências Técnicas

| Área | Tecnologias |
|------|-------------|
| **Linguagens** | Java · Python · C# / .NET · JavaScript · SQL · VBA |
| **Automação & RPA** | Power Automate Desktop · N8N |
| **Low-Code** | Power Apps · Power Automate |
| **Dados & BI** | Power BI · SQL Relacional |

## 💼 Experiência Profissional

### American Bureau of Shipping (ABS)
**Auxiliar de Escritório Aprendiz** · Rio de Janeiro · *Abr/2025 – Presente*

- Desenvolvimento fullstack com **C#**, **.NET** e Visual Studio
- Criação de robôs RPA com **Power Automate Desktop**
- Aplicativos e fluxos com **Power Apps** e **Power Automate**
- Dashboards estratégicos em **Power BI** e implementação de IA em soluções internas

### Subsea7
**Assistente Administrativo Aprendiz** · Rio de Janeiro · *Set/2023 – Jan/2025*

- Automatização do processo de EPIs para toda a região Brasil via Power Platform
- **Resultado:** redução de 70% no tempo de resposta e economia estimada de R$ 800 mil/ano
- Dashboards de Power BI para operações e melhoria operacional
- Gestão de processos via SAP e Pipefy

## 🎓 Formação Acadêmica

- **Tecnólogo em Análise e Desenvolvimento de Sistemas** — Cruzeiro do Sul Virtual *(Nov/2023 – Jun/2026)*
- **Nível Médio Técnico em Administração** — C. E. Sol Nascente *(Fev/2019 – Nov/2021)*

## 📜 Certificações

- JavaScript Completo
- Introdução à Automação com N8N
- Microsoft Power BI
- Produtividade para Programadores
- Design Gráfico

## 🌐 Idiomas

- **Português** — Nativo
- **Inglês** — Básico (Elementary)

## 📂 Sobre Este Projeto

Este repositório contém meu **currículo online interativo**, publicado e acessível em:

🔗 **[curriculum-vitae-fernando-lima-dev.vercel.app](https://curriculum-vitae-fernando-lima-dev.vercel.app/)**

### Stack

- **HTML5** semântico
- **Tailwind CSS** (tema dark com estética tech)
- **Lucide Icons**
- Animações de scroll com **Intersection Observer API**
- Deploy via **Vercel**

### Rodando Localmente

```bash
git clone https://github.com/seu-usuario/Curriculum-Vitae.git
cd Curriculum-Vitae
```

Abra o arquivo `index.html` no navegador — não é necessária nenhuma instalação ou build.

## 📬 Contato

- **Email:** fernandoassisoliveiralima@gmail.com
- **LinkedIn:** [linkedin.com/in/fernandolima-dev](https://www.linkedin.com/in/fernandolima-dev)
- **Telefone:** (21) 96738-5126
- **Localização:** Cachoeiras de Macacu, RJ — Brasil

---

<p align="center">
  Desenvolvido com código, café e automação por <strong>Fernando Lima</strong>.
</p>
